...
input_message = open_ai_models.ChatMessage(content="Is Wagner a good composer?")

request = open_ai_models.ChatRequest(messages=[input_message])

api_response = requests.post("https://api.openai.com/v1/chat/completions", json=request.dict(), headers=HEADERS)

if api_response.status_code == 200:
    response = open_ai_models.ChatResponse.parse_raw(api_response.text)
